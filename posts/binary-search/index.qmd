...
(I actually may be wrong, and the above is just nonsensical rationalization that I may spout during an interview.)
...
#### Intuition
<!-- Describe your first thoughts on how to solve this problem. -->
This is level 3 binary search: the input array is not sorted, but there exists a rule to bisect the array each time. The intuition is as if you are on a mountain: at a position, if the value is smaller than the next one, there must be a peak to the right of it; if the value is larger than the enxt one, there must be a peak to the left of it. Hence, by checking the middle value one at a time, 
