...
- Time complexity:
<!-- Add your time complexity here, e.g. $$O(n)$$ -->
$O(n)$: We need to iterate the list of characters two times. Note that the two times are not nested.

- Space complexity:
<!-- Add your space complexity here, e.g. $$O(n)$$ -->
$O(n)$: We need to create the list of characters
#### Code
```python
import re
...
#### Intuition
<!-- Describe your first thoughts on how to solve this problem. -->
This problem builds upon [Valid Palindrome I](https://leetcode.com/problems/valid-palindrome/). The intuition is this: if we iterate the string with two pointers as before, at the first pair where two characters are different, the string is valid if the substring from *the element right next to the right of the left character in the pair* to *the right character in the pair* is a valid palindrome, or the substring from *the left character in the pair* to *the element right next to the left of the right character in the pair* is a valid palindrome.
...
- Time complexity:
<!-- Add your time complexity here, e.g. $$O(n)$$ -->
$O(n)$: We need to iterate through the input string once in both cases.
