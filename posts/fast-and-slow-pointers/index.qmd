...
However, the problem is not *Is there a cycle?* but *Where is the cycle?*. In other words, we need to find the entrance to the cycle. This bears similarity to [Linked List Cycle II](https://leetcode.com/problems/linked-list-cycle-ii/description/). The solution is to use fast and slow pointers as before, with some insights from math to find the entrance.

![](6-2.png)

We define the following variables (see diagram above):
...
At the intersection point i.e., where two pointers meet, we have the following equations:
...
- Time complexity:
<!-- Add your time complexity here, e.g. $$O(n)$$ -->
$O(n)$: We need to traverse the array a number of time.
- Space complexity:
<!-- Add your space complexity here, e.g. $$O(n)$$ -->
$O(1)$: We only need to store pointers.
...
#### Intuition
<!-- Describe your first thoughts on how to solve this problem. -->
Obviously, the first way is to copy the linked list over to an array, and then use two pointers to check for palindrome. However, to satisfy the $O(1)$ space complexity, we need to check for palindrome in place. The intuition is if a linked list is a palindrome, the second half reversed will be the same as the first half. We can use two pointers to find the middle of the linked list, and then reverse the second half. Finally, we can compare the first half and the second half to check for palindrome. Of course, this will modify the linked list in place, so we need to restore the second half if the linked list is involved in another problem. 
...
- Time complexity:
<!-- Add your time complexity here, e.g. $$O(n)$$ -->
$O(n)$: We need to traverse the linked list a number of time.
